...
def configure_ngrok():
    ngrok_token = "2m2rftBhAiiraoucoSG0oFBqR7N_4nB2T9rKQ8o9Lih4yNJz5"
    try:
...
def update_soul_txt(public_url):
    with open("tata.txt", "w") as file:
        file.write(public_url)
    print(f"New ngrok link saved in tata.txt")
...
def update_vps_soul_txt(public_url):
    ...
    try:
        ...
        sftp = ssh.open_sftp()
        with sftp.open("tata.txt", "w") as file:
            file.write(public_url)
        sftp.close()
        ssh.close()
        print("Updated tata.txt on VPS successfully.")
    except Exception as e:
        print(f"Failed to update tata.txt on VPS: {str(e)}")
